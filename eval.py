...
def eval_model(render_folder_name='eval'):
    ...
    for n in tqdm(range(checkpoint_num)):
        ...
        if only_latest and checkpoint_name != cfg.load_net:
            continue

        model = load_network(checkpoint_name)
        out_folder_name = f'{checkpoint_name}_{render_folder_name}'
        if os.path.isdir(os.path.join(cfg.logdir, out_folder_name)):
            continue

        writer = ImageWriter(output_dir=cfg.logdir, exp_name=out_folder_name)
        ...
        for idx, batch in enumerate(test_loader):
            ...
            swriter.add_scalar(f'PSNR/{checkpoint_name}', psnr, idx)
            swriter.add_scalar(f'SSIM/{checkpoint_name}', ssim, idx)
            swriter.add_scalar(f'LPIPS/{checkpoint_name}', lpips, idx)
            PSNRA.append(psnr)
            SSIMA.append(ssim)
            LPIPSA.append(lpips)
