...
class GridEncoder(nn.Module):
    def __init__(self, input_dim=3, num_levels=16, level_dim=2, per_level_scale=2, base_resolution=16, log2_hashmap_size=19, desired_resolution=2048, gridtype='hash', align_corners=False, interpolation='linear'):
        ...
        self.num_levels = num_levels # num levels, each level multiply resolution by per_level_scale
        ...
        self.gridtype_id = _gridtype_to_id[gridtype] # 'hash': 0, 'tiled': 1
        self.interpolation = interpolation
        self.interp_id = _interp_to_id[interpolation] # 'linear': 0, 'smoothstep': 1
        self.align_corners = align_corners

        # allocate parameters
        offsets = []
        offset = 0
        self.max_params = 2 ** log2_hashmap_size # 2**19
        for i in range(num_levels):
            resolution = int(np.ceil(base_resolution * per_level_scale ** i)) #16
        ...
        offsets.append(offset)
        offsets = torch.from_numpy(np.array(offsets, dtype=np.int32)) #tensor([0, 4920, 18744, 51512, 136696, 352696, 876984, 1401272, 1925560, 2449848, 2974136, 3498424, 4022712, 4547000, 5071288, 5595576, 6119864], dtype=torch.int32)
