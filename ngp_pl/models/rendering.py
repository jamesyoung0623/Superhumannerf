...
def __render_rays_test(model, rays_o, rays_d, hits_t, **kwargs):
    ...
    while samples < kwargs.get('max_samples', MAX_SAMPLES):
        ...
        xyzs, dirs, deltas, ts, N_eff_samples = vren.raymarching_test(rays_o, rays_d, hits_t[:, 0], alive_indices, model.density_bitfield, model.cascades, model.scale, exp_step_factor, model.grid_size, MAX_SAMPLES, N_samples)
        ...
        vren.composite_test_fw(sigmas, rgbs, deltas, ts, hits_t[:, 0], alive_indices, kwargs.get('T_threshold', 1e-4), N_eff_samples, opacity, depth, rgb)
...
def __render_rays_train(model, rays_o, rays_d, hits_t, **kwargs):
    ...
    (results['vr_samples'], results['opacity'], results['depth'], results['rgb'], results['ws']) = VolumeRenderer.apply(sigmas, rgbs.contiguous(), results['deltas'], results['ts'], rays_a, kwargs.get('T_threshold', 1e-4))
